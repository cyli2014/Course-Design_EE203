...
for m = 1:samples
	...
	soft_state = zeros(4,1);
	hard_state = zeros(4,41);
	inter_state = zeros(4,27);
	...
	for life = 1:87600
		...
		sum_comp = sum(soft_state(:)) + sum(hard_state(:)) + sum(inter_state(:)) + sum(col_state(:));
		...
		sum_state = sys_state(2) + sys_state(3) + sys_state(4);				
					
		if (sys_state(1) || sys_state(5) || sum_state >= 2)
			srs_fail_flag_new = 1;
